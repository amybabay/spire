...
for i in {1..18}; do
    ...
    docker cp "$container:/app/spire/prime/bin/logs/prime.log" "$DEST_DIR/prime_3_$i.log" 2>/dev/null \
        && echo "  prime$i.log copied." || echo "  Warning: prime.log not found in $container."

    docker cp "$container:/app/spire/prime/bin/logs/sm.log" "$DEST_DIR/sm_3_$i.log" 2>/dev/null \
